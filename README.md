...
* ↔️ **Протокол VLESS gRPC (Порт 8443 TCP)**: Надежный резервный канал поверх gRPC и TLS с поддержкой мультиплексирования.
...
## 🏷️ Формат профилей и эмодзи в подписках

При импорте подписки в клиент автоматически добавляются следующие локации:

| Протокол | Формат имени в клиенте | Порт и транспорт | Назначение |
|---|---|---|---|
| **VLESS TCP** | `🇳🇱🌐 VLESS-TCP` | `443/tcp` (XTLS-Vision, TLS 1.3) | Основной высокоскоростной протокол для повседневного использования |
| **VLESS gRPC** | `🇳🇱↔️ VLESS-gRPC` | `8443/tcp` (gRPC, HTTP/2) | Запасной/резервный канал в сетях с блокировками Vision |
| **Hysteria 2** | `🇳🇱⚡ Hysteria2` | `443/udp` (QUIC, UDP) | Ультра-низкая задержка и обход потерь пакетов на мобильном интернете |
| **VLESS Reality** | `🇳🇱🪞 VLESS-Reality` | `443/tcp` (Reality Masking) | Маскировка под реальный веб-сайт (при включении через меню `xry`) |

---

## 📱 Совместимость с клиентскими приложениями

Сгенерированные ссылки и сервер подписок на 100% совместимы с приложениями:
...
В репозитории настроен автоматический CI-конвейер GitHub Actions. Для локального запуска тестов:
