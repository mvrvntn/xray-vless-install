...
- [🏷️ Формат профилей и эмодзи в подписках](#️-формат-профилей-и-эмодзи-в-подписках)
- [📱 Совместимость с клиентскими приложениями](#-совместимость-с-клиентскими-приложениями)
- [🛠️ Быстрый старт](#️-быстрый-старт)
- [🖥️ Дашборд управления xry](#️-дашборд-управления-xry)
- [⚙️ Дополнительные режимы запуска](#️-дополнительные-режимы-запуска)
- [🤝 Совместимость с AntiZapret-VPN](#-совместимость-с-antizapret-vpn)
...
3. **Открытые порты**:
   ...
   - `20000:50000/udp` — диапазон прыгающих портов (Port Hopping) для Hysteria 2.
...
* **Причина**: На сервере уже запущен веб-сервер (Nginx, Apache, Caddy).
* **Решение**: Остановите службу (`systemctl stop nginx`) перед установкой, либо освободите 80 порт.

### 2. Hysteria 2 не подключается на Oracle Cloud / Hetzner
* **Причина**: В облачной панели провайдера закрыты входящие UDP-порты (Security Lists / Firewall).
* **Решение**: В панели хостинга добавьте разрешающие правила для `443/udp` и диапазона `20000-50000/udp`.

### 3. Как обновить ядро Xray или применить свежие обновления скрипта?
